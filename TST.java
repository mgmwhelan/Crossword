...
public class TST<Value> {
    ...
    public boolean keyThatMatches(String pattern) {
        return collect(root, new StringBuilder(), 0, pattern); 
    }

    private boolean collect(Node<Value> x, StringBuilder prefix, int i, String pattern) {
        if (x == null) return false;      
        char c = pattern.charAt(i);
        if (c == '.' || c < x.c) 
            if (collect(x.left, prefix, i, pattern))
                return true; 
        if (c == '.' || c == x.c) {
            if (i == pattern.length() - 1 && x.val != null) return true;
            if (i < pattern.length() - 1) {
                if (collect(x.mid, prefix.append(x.c), i+1, pattern))
                    return true;
                prefix.deleteCharAt(prefix.length() - 1);
            }
        }
        if (c == '.' || c > x.c) return collect(x.right, prefix, i, pattern);
        return false; 
    }
